...
const Navbar = () => {
    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto px-6 lg:px-12 flex justify-between items-center">
                ...
                <div className="hidden md:flex items-center gap-8">
                    ...
                    <div className="flex items-center gap-3">
                        {[
                            { icon: <Facebook size={16} />, link: "https://facebook.com/NoorPublicSchool", color: "hover:bg-[#1877F2]" },
                            { icon: <Instagram size={16} />, link: "https://instagram.com/noor_public_school_karachi", color: "hover:bg-[#D62976]" },
                            { icon: <Linkedin size={16} />, link: "https://linkedin.com/in/noorpublicschool", color: "hover:bg-[#0a66c2]" }
                        ].map((social, i) => (
                            <a
                                key={i}
                                href={social.link}
            ...
            {/* Mobile Menu Overlay */}
            <div className={`fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[-1] transition-opacity duration-300 md:hidden ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                ...
                <div className="p-8 flex flex-col h-full">
                    ...
                    <div className="space-y-4">
                        {navLinks.map((link) => (
                            <Link
                                ...
                                className={`block text-xl font-bold tracking-tight transition-colors ${location.pathname === link.path ? "text-amber-400" : "text-white hover:text-amber-200"
                                    }`}
                            >
                                {link.name}
                            </Link>
                    ...
                    <div className="mt-auto pt-8 border-t border-white/10">
                        ...
                        <div className="flex gap-6 text-white/70">
                            {
                                [
                                    { icon: <Facebook size={16} />, link: "https://facebook.com/NoorPublicSchool", color: "bg-[#1877F2]" },
                                    { icon: <Instagram size={16} />, link: "https://instagram.com/noor_public_school_karachi", color: "bg-[#D62976]" },
                                    { icon: <Linkedin size={16} />, link: "https://linkedin.com/in/noorpublicschool", color: "bg-[#0a66c2]" }
                                ].map((social, i) => (
                                    <a
                                        key={i}
                                        href={social.link}
                                        target="_blank"
                                        rel="noreferrer"
                                        className={`p-2 ${social.color} rounded-full transition-all duration-300 border text-white border-slate-200"
                                            } ${social.color}`}
                                    >
                                        {social.icon}
                                    </a>
