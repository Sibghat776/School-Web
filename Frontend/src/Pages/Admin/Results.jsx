...
const AdminResults = () => {
  ...
  const [filters, setFilters] = useState({ className: '', examType: '' });
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await resultService.getClass(params);
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <PanelCard title="New Result">
          <form onSubmit={create} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Input name="examType" value={form.examType} onChange={onChange} placeholder="Exam (e.g. Mid)" required />
            ...
            <Input name="session" value={form.session} onChange={onChange} placeholder="Session (e.g. 2025-26)" className="md:col-span-3" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Input value={filters.className} onChange={(e) => setFilters({ ...filters, className: e.target.value })} placeholder="Filter by class" />
        <Input value={filters.examType} onChange={(e) => setFilters({ ...filters, examType: e.target.value })} placeholder="Filter by exam" onKeyDown={(e) => e.key === 'Enter' && load()} />
      </div>
